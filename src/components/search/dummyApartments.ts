...
function generateRoomDetails(rooms: number, size: number): RoomDetail[] {
  ...
  return [
    ...
    {
      name: 'Balcony',
      size: Math.round(size * 0.07 * 10) / 10,
      icon: 'balcony',
    },
    {
      name: 'Bathroom',
      size: Math.round(size * 0.06 * 10) / 10,
      icon: 'bathroom',
    },
    {
      name: 'Kitchen',
    ...
    {
      name: 'Storage',
      size: Math.round(size * 0.04 * 10) / 10,
      icon: 'storage',
    },
    {
      name: 'Living room',
      size: Math.round(size * 0.18 * 10) / 10,
      icon: 'living',
    },
...
export const DUMMY_APARTMENTS: ApartmentData[] = Array.from(
  { length: 400 },
  (_, i) => {
    const rooms = (i % 5) + 1;
    ...
    return {
      ...
      // Content
      description:
        `${rooms}-bedroom apartment in the ${complex} complex, block ${block}, floor ${floor}. ` +
