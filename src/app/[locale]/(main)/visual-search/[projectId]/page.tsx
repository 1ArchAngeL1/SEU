...
import { Loader2, ArrowLeft, ZoomIn, ZoomOut, Maximize } from 'lucide-react';
        ...
        {renderImage && (
          <>
            ...
            <div className="hidden lg:block relative w-full rounded-2xl overflow-hidden border border-pale-gray/10">
              ...
              {/* Hovered building info tooltip */}
              {hoveredId && (() => {
                const b = buildings.find((b) => b.id === hoveredId);
                if (!b) return null;
                const center = b.polygon && b.polygon.length >= 3
                  ? { x: b.polygon.reduce((s, p) => s + p.x, 0) / b.polygon.length, y: b.polygon.reduce((s, p) => s + p.y, 0) / b.polygon.length }
                  : null;
                return center ? (
                  <div
